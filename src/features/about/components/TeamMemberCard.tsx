...
export function TeamMemberCard({ member, size = "md" }: TeamMemberCardProps) {
  ...
  return (
    <div>
      <div
        className={`aspect-square overflow-hidden blob-shape-1 bg-muted flex items-center justify-center mx-auto ${
          isLarge ? "w-40 md:w-48 mb-4" : "w-28 md:w-32 mb-3"
        }`}
      >
        {member.imageUrl ? (
      ...
      <div className={isLarge ? "px-2 text-center" : "px-1 text-center"}>
        ...
        <p
          className={`font-sans text-xs uppercase tracking-wide ${isLarge ? "text-primary mb-2" : "text-destructive"}`}
        >
          {member.title}
        </p>
      </div>
