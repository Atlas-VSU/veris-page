  ...
  return (
    <>
      <section className="px-4 py-8 max-w-[1200px] mx-auto font-sans space-y-10 animate-fade-in-up animation-delay-200">
        <h2 className="font-serif text-center text-4xl font-medium text-neutral-900">
          Featured Platform
        </h2>

        <div className="bg-white rounded-[2.5rem] shadow-[0_20px_60px_rgba(0,0,0,0.12)] border border-zinc-200/80 flex flex-col lg:flex-row p-10 lg:p-12 gap-12 items-center">
          {/* Left Content */}
      ...
      <FeaturedPlatformModal
        ...
        onNext={() =>
          setCurrentSlideIndex(
            (prev) => (prev + 1) % featuredData.photos.length,
          )
        }
        onPrev={() =>
          setCurrentSlideIndex((prev) =>
            prev === 0 ? featuredData.photos.length - 1 : prev - 1,
          )
        }
      />
    </>
